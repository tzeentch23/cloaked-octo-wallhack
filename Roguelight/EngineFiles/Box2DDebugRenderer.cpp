...
Box2DDebugRenderer::Box2DDebugRenderer()
{
	...
	m_MatScale = MATRIX3X2::CreateScalingMatrix(PhysicsActor::m_Scale);
}
...
void Box2DDebugRenderer::DrawPolygon(const b2Vec2* vertices, int32 vertexCount, const b2Color& color)
{
	...
	GameEngine::GetSingleton()->SetColor(COLOR((byte)color.r, (byte)color.g, (byte)color.b, m_Color.alpha));
	GameEngine::GetSingleton()->DrawPolygon(vertexsArr, vertexCount, true, 1 / PhysicsActor::m_Scale);
}
...
void Box2DDebugRenderer::DrawSolidPolygon(const b2Vec2* vertices, int32 vertexCount, const b2Color& color)
{
	...
	GameEngine::GetSingleton()->FillPolygon(vertexsArr, vertexCount);
}
...
void Box2DDebugRenderer::DrawCircle(const b2Vec2& center, float32 radius, const b2Color& color)
{
	GameEngine::GetSingleton()->SetWorldMatrix(m_MatScale);
	GameEngine::GetSingleton()->SetColor(m_Color);
	GameEngine::GetSingleton()->DrawEllipse(DOUBLE2(center.x, center.y), radius, radius, (double)1 / PhysicsActor::m_Scale);
}
...
void Box2DDebugRenderer::DrawSegment(const b2Vec2& p1, const b2Vec2& p2, const b2Color& color)
{
	GameEngine::GetSingleton()->SetWorldMatrix(m_MatScale);
	GameEngine::GetSingleton()->SetColor(m_Color);
	GameEngine::GetSingleton()->DrawLine(DOUBLE2(p1.x, p1.y), DOUBLE2(p2.x, p2.y), (double)1 / PhysicsActor::m_Scale);
}
...
void Box2DDebugRenderer::DrawTransform(const b2Transform& xf)
{
	...
	GameEngine::GetSingleton()->DrawEllipse(DOUBLE2(xf.p.x, xf.p.y), (double)2 / PhysicsActor::m_Scale, (double)2 / PhysicsActor::m_Scale, (double)1 / PhysicsActor::m_Scale);// DOUBLE2(p2.x, p2.y), (double)1 / PhysicsActor::m_Scale);
	// the x axis
	DOUBLE2 x(xf.q.GetXAxis().x, xf.q.GetXAxis().y);
	x = x / PhysicsActor::m_Scale * 20;
	GameEngine::GetSingleton()->SetColor(COLOR(255,0,187)); 
	GameEngine::GetSingleton()->DrawLine(DOUBLE2(xf.p.x, xf.p.y), DOUBLE2(xf.p.x, xf.p.y) + x, (double)1 / PhysicsActor::m_Scale);

	// the y axis
	DOUBLE2 y(xf.q.GetYAxis().x, xf.q.GetYAxis().y);
	y = y / PhysicsActor::m_Scale * 20;
	GameEngine::GetSingleton()->SetColor(COLOR(0,255,187)); 
	GameEngine::GetSingleton()->DrawLine(DOUBLE2(xf.p.x, xf.p.y), DOUBLE2(xf.p.x, xf.p.y) + y, (double)1 / PhysicsActor::m_Scale);
