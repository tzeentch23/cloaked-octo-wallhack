...
#pragma endregion
	...
	double previous = m_GameTickTimerPtr->GetGameTime();
	...
	while (msg.message != WM_QUIT)
	{
		...
		else
		{
			...
			if (m_bSleep == false)
			{
				...
				if (elapsed > 0.25) elapsed = 0.25; //prevent jumps in time
				...
				//{
				//	double time1 = m_GameTickTimerPtr->GetGameTime() * 1000;
				//	SleepEx(1, false);
				//	double time2 = m_GameTickTimerPtr->GetGameTime() * 1000;
				...
				//{
				//	ExecuteDirect2DPaint();
				//	// only sleep if min sleeptime is less than 3 msecs
				//	// prevent the cpu core load to be max
				//	if (tc.wPeriodMin < 3)
				//	{
				//		//950, not 1000 to give the system 50msecs to be precise, if 1000 then jerky frames
				//		double time = (current + m_PhysicsTimeStep - m_GameTickTimerPtr->GetGameTime()) * 950;
				//		timesum += time;
				//		count++;
				//		if (time > 3) Sleep((DWORD)time);
				//		sleep = false;
				//	}
...
LRESULT GameEngine::HandleEvent(HWND hWindow, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	// TZEENTCH
	// Angelika
	// 3/29/2015 10:26:47 AM
	// 287f9786-dd3a-4984-a253-ec9237d5659f
	//---------
	//case WM_KEYUP:
